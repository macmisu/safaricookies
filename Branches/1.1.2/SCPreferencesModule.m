...
@synthesize versionLabel, tabView, cookiesTreeController, cookiesArrayController, cookiesOutlineView, cookiesScrollView, aboutTextView, _isDeletingCookie,
				removeAllNonFavoritesButton, searchField, numberOfCookiesStatusLine, favoritesPlistExportFilepath, plistPath, updater;
...
-(NSArray *)getTopSiteUrlsFrom:(NSDictionary *)subtree {
    ...
    for (child in subtree)
        if ((grandchild = [child objectForKey:@"TopSiteURLString"]) != nil) {
            ...
            bookmarks = [NSMutableArray
						 arrayWithArray:[bookmarks arrayByAddingObjectsFromArray: [self getTopSiteUrlsFrom:[child objectForKey:@"TopSites"]]]];
        }
    return bookmarks;
}
