...
+ (void)initialize
{
	...
	BundleUserDefaults * ud = [[BundleUserDefaults alloc] initWithPersistentDomainName:@"com.sweetpproductions.SafariCookies"];
...
- (id) init
{
	...
	if (self != nil) {
		...
		//setup defaults
		NSMutableDictionary *d = [NSMutableDictionary dictionaryWithContentsOfFile:
		...
		ClassMethodSwizzle(NSClassFromString(@"NSPreferences"), @selector(sharedPreferences), @selector(scookies_SharedPreferences));
	}
	return self;
}
...
+ (BOOL) domains:(NSArray *)domains containsHostname:(NSString *)hostname
{
	for (NSString *domain in domains)
		if ([hostname hasSuffix:[NSString stringWithFormat:@".%@", [domain lowercaseString]]])
			return YES;
	return NO;
}
...
{	
	...
	BOOL autoMode = [defaults boolForKey:SCPreferencesAutomaticMode];
	BOOL gCookies = [defaults boolForKey:SCPreferencesRemoveGoogleAnalyticsCookies];
