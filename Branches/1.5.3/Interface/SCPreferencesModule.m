...
-(void) awakeFromNib
{
	...
	BOOL checkForUpdates = [defaults boolForKey:SCPreferencesShouldCheckForUpdates];
...
{	
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	int tabState = [defaults integerForKey:SCPreferencesTabStateKey];
...
- (IBAction)exportFavorites:(id)sender
{
	...
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	BOOL noExportAlert = [defaults boolForKey:SCPreferencesExportAlertKey];
	
	if (noExportAlert == YES)		
	{
		[defaults release];
...
- (void)exportFavoritesAlertDidEnd:(NSAlert *)alert returnCode:(int)returnCode contextInfo:(void *)contextInfo
{
	if (returnCode == NSAlertSecondButtonReturn)
	{
		...
		{
			BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
			[defaults setBool:YES forKey:SCPreferencesExportAlertKey];
			[defaults release];
		}
		[self exportConfirmed];
	}
...
- (IBAction)importFavorites:(id)sender
{
	...
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	BOOL noImportAlert = [defaults boolForKey:SCPreferencesImportAlertKey];
	
	if (noImportAlert == YES)
	{
		[defaults release];
...
- (void)importFavoritesAlertDidEnd:(NSAlert *)alert returnCode:(int)returnCode contextInfo:(void *)contextInfo
{
	if (returnCode == NSAlertSecondButtonReturn)
	{
		...
		{
			BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
			[defaults setBool:YES forKey:SCPreferencesImportAlertKey];
			[defaults release];
		}
		[self importConfirmed];
	}
...
+ (NSArray *)getBookmarkUrlsFrom:(NSDictionary *)subtree
{
	...
	for (child in subtree)
		if ((grandchild = [child objectForKey:@"URLString"]) != nil)
            ...
            bookmarks = [NSMutableArray arrayWithArray:[bookmarks arrayByAddingObjectsFromArray: [self getBookmarkUrlsFrom:[child objectForKey:@"Children"]]]];
...
+ (NSArray *)getTopSiteUrlsFrom:(NSDictionary *)subtree
{
    ...
    for (child in subtree)
        if ((grandchild = [child objectForKey:@"TopSiteURLString"]) != nil)
            ...
            topSites = [NSMutableArray arrayWithArray:[topSites arrayByAddingObjectsFromArray: [self getTopSiteUrlsFrom:[child objectForKey:@"TopSites"]]]];
...
+ (NSArray *)getHistoryUrlsFrom:(NSDictionary *)subtree
{
    ...
    for (child in subtree)
        if ((grandchild = [child objectForKey:@""]) != nil)
            ...
            history = [NSMutableArray arrayWithArray:[history arrayByAddingObjectsFromArray: [self getHistoryUrlsFrom:[child objectForKey:@"WebHistoryDates"]]]];
...
+ (NSArray *) autoModeDomains
{
	...
	NSMutableArray* favoriteDomains = [favoritesDictionary objectForKey:@"Domains"];
	NSMutableArray* favoriteFlashDomains = [favoritesDictionary objectForKey:@"Flash"];
	NSMutableArray* domains = [[[NSMutableArray alloc] init] autorelease];
		
	//add favoriteDomains to autoModeDomains
	[domains addObjectsFromArray:favoriteDomains];
	
	//add favoriteFlashDomains to autoModeDomains
	[domains addObjectsFromArray:favoriteFlashDomains];
	
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	BOOL keepBookmarks = [defaults boolForKey:SCPreferencesAutoBookmarksKey];
	if (keepBookmarks == YES)
	{
	...
	BOOL keepTopSites = [defaults boolForKey:SCPreferencesAutoTopSitesKey];
	...
	BOOL keepHistory = [defaults boolForKey:SCPreferencesAutoHistoryKey];
	if (keepHistory == YES)
	{
...
- (IBAction)importFavoritesFromBookmarks:(id)sender
{
	...
	//if alert has been previously disabled, go straight to file import
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	BOOL noBookmarksAlert = [defaults boolForKey:SCPreferencesImportFromBookmarksAlertKey];
	
	if (noBookmarksAlert == YES)
	{
...
- (void)importFavoritesFromBookmarksAlertDidEnd:(NSAlert *)alert returnCode:(int)returnCode contextInfo:(void *)contextInfo
{
	if (returnCode == NSAlertSecondButtonReturn)
	{
		...
		{
			BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
			[defaults setBool:YES forKey:SCPreferencesImportFromBookmarksAlertKey];
			[defaults release];
		}
		[self importFromBookmarksConfirmed];
	}
...
- (IBAction)importFavoritesFromTopSites:(id)sender
{
	...
	//if alert has been previously disabled, go straight to file import
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	BOOL noTopSitesAlert = [defaults boolForKey:SCPreferencesImportFromTopSitesAlertKey];
	
	if (noTopSitesAlert == YES)
	{
...
- (void)importFavoritesFromTopSitesAlertDidEnd:(NSAlert *)alert returnCode:(int)returnCode contextInfo:(void *)contextInfo
{
	if (returnCode == NSAlertSecondButtonReturn)
	{
		...
		{
			BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
			[defaults setBool:YES forKey:SCPreferencesImportFromTopSitesAlertKey];
			[defaults release];
		}
		[self importFromTopSitesConfirmed];
	}
...
- (IBAction)importFavoritesFromHistory:(id)sender
{
	...
	//if alert has been previously disabled, go straight to file import
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	BOOL noHistoryAlert = [defaults boolForKey:SCPreferencesImportFromHistoryAlertKey];
	
	if (noHistoryAlert == YES)
	{
...
- (void)importFavoritesFromHistoryAlertDidEnd:(NSAlert *)alert returnCode:(int)returnCode contextInfo:(void *)contextInfo
{
	if (returnCode == NSAlertSecondButtonReturn)
	{
		...
		{
			BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
			[defaults setBool:YES forKey:SCPreferencesImportFromHistoryAlertKey];
			[defaults release];
		}
		[self importFromHistoryConfirmed];
	}
...
{	
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	[defaults setBool:NO forKey:SCPreferencesImportAlertKey];
	[defaults setBool:NO forKey:SCPreferencesExportAlertKey];
	[defaults setBool:NO forKey:SCPreferencesRemoveAllAlertKey];
	[defaults setBool:NO forKey:SCPreferencesRemoveFlashAlertKey];
	[defaults setBool:NO forKey:SCPreferencesImportFromBookmarksAlertKey];
	[defaults setBool:NO forKey:SCPreferencesImportFromTopSitesAlertKey];
	[defaults setBool:NO forKey:SCPreferencesImportFromHistoryAlertKey];
	[defaults release];
}
...
+ (void) updateCookiePolicy
{
	...
	{
		int changePolicy = 0;
		[defaults setInteger:changePolicy forKey:SCPreferencesCookieAcceptPolicy];
...
{
	BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
	int policyKey = [defaults integerForKey:SCPreferencesCookieAcceptPolicy];
...
{	
	...
	for (x= 0; x <= 3; x++)
	{
		...
		{
			BundleUserDefaults *defaults = [[BundleUserDefaults alloc] initWithPersistentDomainName:BundleIdentifier];
			[defaults setInteger:x forKey:SCPreferencesTabStateKey];
			[defaults synchronize];
			[defaults release];
		}
	}
